...
- (ResumeCacheData *)getResumeData:(NSString *)filePath {
    ...
    NSString *session = [sessionDic objectForKey:fileMD5Key];
    NSInteger expireTime = [[timeDic objectForKey:fileMD5Key] integerValue];
    unsigned long long lastModTime = [[lastModTimeDic objectForKey:fileMD5Key] unsignedLongLongValue];
    unsigned long long coverLastModTime = [[coverLastModTimeDic objectForKey:fileMD5Key] unsignedLongLongValue];
    NSString *sResumeData = [resumeDataDic objectForKey:fileMD5Key];
    NSInteger nowTime = (NSInteger)[[NSDate date] timeIntervalSince1970] + 1;
...
- (void)saveSession:(NSString *)filePath withSessionKey:(NSString *)vodSessionKey withResumeData:(NSData *)resumeData
    ...
    if (vodSessionKey == nil || vodSessionKey.length == 0 || resumeData == nil || resumeData.length == 0 || lastModTime == 0) {
        [sessionDic removeObjectForKey:fileMD5Key];
        [timeDic removeObjectForKey:fileMD5Key];
        [lastModTimeDic removeObjectForKey:fileMD5Key];
        [coverLastModTimeDic removeObjectForKey:fileMD5Key];
        [resumeDataDic removeObjectForKey:fileMD5Key];
    } else {
        [sessionDic setValue:vodSessionKey forKey:fileMD5Key];
        [timeDic setValue:@(expireTime) forKey:fileMD5Key];
        [lastModTimeDic setValue:@(lastModTime) forKey:fileMD5Key];
        [coverLastModTimeDic setValue:@(coverLastModTime) forKey:fileMD5Key];
        NSString *sResumeData = [resumeData base64EncodedStringWithOptions:0];
        [resumeDataDic setValue:sResumeData forKey:fileMD5Key];
    }
